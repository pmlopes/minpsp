...
#ifdef __WIN32
#define PATH_SUFFIX "/bin/libmikmod-config.exe"
#else
#define PATH_SUFFIX "/bin/libmikmod-config"
#endif
/************************/
...
static enum SDLConfigMode g_configmode;
...
int process_args(int argc, char **argv)
{
	...
	g_configmode = SDL_CONFIG_UNKNOWN;
	...
	while(ch != -1)
	{
		switch(ch)
		{
			case 'p' : g_configmode = SDL_CONFIG_PREFIX;
					   ret = 1;
					   break;
			case 'e' : g_configmode = SDL_CONFIG_EXEC_PREFIX;
					   ret = 1;
					   break;
			case 'v' : g_configmode = SDL_CONFIG_VERSION;
					   ret = 1;
					   break;
			case 'c' : g_configmode = SDL_CONFIG_CFLAGS;
					   ret = 1;
					   break;
			case 'l' : g_configmode = SDL_CONFIG_LIB;
					   ret = 1;
					   break;
			case 'L' : g_configmode = SDL_CONFIG_LDADD;
					   ret = 1;
					   break;
...
{
	fprintf(stderr, "Usage: libmikmod-config [--prefix[=DIR]] [--exec-prefix[=DIR]] [--version] [--cflags] [--libs] [--ldadd]\n");
}
...
char *find_path(char *name)
{
	...
	*(ptr) = '\0';
	...
	if(found)
	{
		...
		{
			fprintf(stderr, "Error, path not large enough for creating the libmikmod path\n");
		}
	...
	if (pspdev_env != NULL) {
		switch(g_configmode)
		{
			...
			case SDL_CONFIG_CFLAGS :
				...
				break;
			case SDL_CONFIG_LIB:
				printf("-L%s/psp/lib -lmikmod ", pspdev_env);
				break;
			case SDL_CONFIG_LDADD :
				break;
