...
#ifdef __WIN32
#define PATH_SUFFIX "/bin/ode-config.exe"
#else
#define PATH_SUFFIX "/bin/ode-config"
#endif
/************************/
	...
	while(ch != -1)
	{
		switch(ch)
		{
			...
			case 'c' : g_configmode = CONFIG_CFLAGS;
					   ret = 1;
					   break;
			case 'l' : g_configmode = CONFIG_LIBS;
					   ret = 1;
					   break;
		...
		// fetch the next item
		ch = getopt_long(argc, argv, "pevcl", arg_opts, NULL);
	}
...
{
	fprintf(stderr, "Usage: ode-config [--prefix[=DIR]] [--exec-prefix[=DIR]] [--version] [--cflags] [--libs]\n");
}
...
char *find_path(char *name)
{
	...
	*(ptr) = '\0';
	...
	if(found)
	{
		...
		{
			fprintf(stderr, "Error, path not large enough for creating the ODE path\n");
		}
	...
	if (pspdev_env != NULL) {
		switch(g_configmode)
		{
			...
			case CONFIG_VERSION :
				...
				exit(0);
			case CONFIG_CFLAGS :
				printf("-I%s/psp/include -DdSINGLE ", pspdev_env);
				break;
			case CONFIG_LIBS:
				printf("-L%s/psp/lib -lode ", pspdev_env);
				break;
