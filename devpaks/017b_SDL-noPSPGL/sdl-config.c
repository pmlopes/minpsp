...
static struct option arg_opts[] = 
{
	...
	{"static-libs", no_argument, NULL, 'L'},
	{ NULL, 0, NULL, 0 }
};
	...
	while(ch != -1)
	{
		switch(ch)
		{
			...
			case 'l' : g_configmode = SDL_CONFIG_LIB;
					   ret = 1;
					   break;
			case 'L' : g_configmode = SDL_CONFIG_STATIC_LIB;
					   ret = 1;
					   break;
...
{
	fprintf(stderr, "Usage: sdl-config [--prefix[=DIR]] [--exec-prefix[=DIR]] [--version] [--cflags] [--libs] [--static-libs]\n");
}
